# feedForward.cpp - reads sensors, actions, an individual
...
'''*******************************************************************************
This function does a neural net feed-forward operation, sensor (input) neurons
...
There is no back-propagation in self simulator. Once an individual's neural net
brain is wired at birth, weights and topology do not change during the
individual's lifetime.

The data structure Indiv.neurons contains internal neurons, Indiv.connections
...
We have three types of neurons:
     ...
     internal neurons - each takes inputs from sensors or other internal neurons
         ...
         one simStep to the next. (For example, neuron that feeds itself will use
     ...
     action (output) neurons - each takes inputs from sensors or other internal
         neurons; In self function, has an output value in an arbitrary range
...
std.array<float, Indiv.feedForward(unsigned simStep)
    ...
    for (Gene & conn : nnet.connections)
        if conn.sinkType == ACTION and not neuronOutputsComputed:
            # We've handled all the connections from sensors and now we are about to
            # start on the connections to the action outputs, now it's time to
            # update and latch all the neuron outputs to their proper range (-1.0..1.0)
            for (neuronIndex = 0; neuronIndex < nnet.neurons.size(); ++neuronIndex)
        ...
        # The values are summed for now, passed through a transfer function
        float inputVal
