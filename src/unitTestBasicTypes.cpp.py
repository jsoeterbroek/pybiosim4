...
# This tests the types Dir, Coord, Polar, enum Compass.
...
# 4 digits accuracy
def areClosef(self, a, b):
    return std.abs(a - b) < 0.0001



def unitTestBasicTypes(self):
    # Dir
    ...
    # .asNormalizedCoord()
    c1 = Dir(Compass.CENTER).asNormalizedCoord()
    ...
    # .asNormalizedPolar()
    d1 = Compass.SW
    p1 = d1.asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.SW)
    p1 = Dir(Compass.S).asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.S)
    p1 = Dir(Compass.SE).asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.SE)
    p1 = Dir(Compass.W).asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.W)
    p1 = Dir(Compass.E).asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.E)
    p1 = Dir(Compass.NW).asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.NW)
    p1 = Dir(Compass.N).asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.N)
    p1 = Dir(Compass.NE).asNormalizedPolar()
    assert(p1.mag == 1 and p1.dir == Compass.NE)
    ...
    assert(Coord(-1, -1).isNormalized())
    assert(not Coord(0, 2).isNormalized())
    assert(not Coord(1, 2).isNormalized())
    assert(not Coord(-1, 2).isNormalized())
    assert(not Coord(-2, 0).isNormalized())
    ...
    assert(p1.mag == 0 and p1.dir == Compass.CENTER)
    p1 = Coord(0, 1).asPolar()
    assert(p1.mag == 1 and p1.dir == Compass.N)
    p1 = Coord(-10, -10).asPolar()
    assert(p1.mag == 14 and p1.dir == Compass.SW); # round down mag
    p1 = Coord(100, 1).asPolar()
    assert(p1.mag == 100 and p1.dir == Compass.E); # round down mag
    ...
    assert(p1.mag == 0 and p1.dir == Compass.S)
    p1 = Polar(10, Compass.SE)
    assert(p1.mag == 10 and p1.dir == Compass.SE)
    p1 = Polar(-10, Compass.NW)
    assert(p1.mag == -10 and p1.dir == Compass.NW)
