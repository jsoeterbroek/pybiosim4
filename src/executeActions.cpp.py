...
# Given a factor in the range 0.0..1.0, a bool with the
# probability of it being True proportional to factor. For example, if
# factor == 0.2, there is a 20% chance self function will
# return True.
def prob2bool(self, factor):
...
step (simStep). After evaluating the action neuron outputs, function is
...
eventually be converted in self function to a probability 0.0..1.0 of actually
getting executed.

For the various possible action neurons, they are driven by a sufficiently
strong level, do self:
    ...
    SET_RESPONSIVENESS action - immediately change indiv.responsiveness to the action
         level scaled to 0.0..1.0 (because we have exclusive access to self member in
         our own individual during self function)
...
def executeActions(self, &indiv, std.array<float, &actionLevels):
    # Only a subset of all possible actions might be enabled (i.e., in).
    ...
    # 2..4*p.stepsPerGeneration. If self action neuron is enabled but not driven,
    # will default to 1.5 + e^(3.5) = a period of 34 simSteps.
    if isEnabled(Action.SET_OSCILLATOR_PERIOD):
        periodf = actionLevels[Action.SET_OSCILLATOR_PERIOD]
        newPeriodf01 = (std.tanh(periodf) + 1.0) / 2.0; # convert to 0.0..1.0
        newPeriod = 1 + (int)(1.5 + std.exp(7.0 * newPeriodf01))
    ...
    # If self action neuron is enabled but not driven, default to
    # mid-level period of 17 simSteps.
    if isEnabled(Action.SET_LONGPROBE_DIST):
        constexpr maxLongProbeDistance = 32
        level = actionLevels[SET_LONGPROBE_DIST]
        level = (std.tanh(level) + 1.0) / 2.0; # convert to 0.0..1.0
        level = 1 + level * maxLongProbeDistance
        indiv.longProbeDist = (unsigned)level


    # Emit signal0 - if self action value is below a threshold, emitted.
    # Otherwise convert the action value to a probability of emitting one unit of
    # signal (pheromone).
    # Pheromones may be emitted immediately (see signals.cpp). If self action neuron
    # is enabled but not driven, will be emitted.
    if isEnabled(Action.EMIT_SIGNAL0):
        constexpr emitThreshold = 0.5;  # 0.0..1.0; 0.5 is midlevel
        level = actionLevels[Action.EMIT_SIGNAL0]
        level = (std.tanh(level) + 1.0) / 2.0; # convert to 0.0..1.0
        level *= responsivenessAdjusted
        if level > emitThreshold and prob2bool(level):
            signals.increment(0, indiv.loc)



    # Kill forward -- if self action value is > threshold, is converted to probability
    # of an attempted murder. Probabilities under the threshold are considered 0.0.
    # If self action neuron is enabled but not driven, neighbors are safe.
    if isEnabled(Action.KILL_FORWARD) and p.killEnable:
        constexpr killThreshold = 0.5;  # 0.0..1.0; 0.5 is midlevel
        level = actionLevels[Action.KILL_FORWARD]
        level = (std.tanh(level) + 1.0) / 2.0; # convert to 0.0..1.0
        level *= responsivenessAdjusted
        if level > killThreshold and prob2bool((level - ACTION_MIN) / ACTION_RANGE):
            otherLoc = indiv.loc + indiv.lastMoveDir
            if grid.isInBounds(otherLoc) and grid.isOccupiedAt(otherLoc):
    ...
    # X and Y components of all the movement urges, pass the X and Y sums through
    # a transfer function (tanh()) to get a range -1.0..1.0. The absolute values of the
    # X and Y values are passed through prob2bool() to convert to -1, 0, 1, then
    # multiplied by the component's signum. This results in the x and y components of
    # a normalized movement offset. I.e., probability of movement in either
    # dimension is the absolute value of tanh of the action level X, components and
    # the direction is the sign of the X, components. For example, a particular
    # action neuron:
    ...
    #     X, Y == -1, after applying the sign and probability
    #     The agent will then be moved West (an offset of -1, 0) if it's a legal move.

    float level
    Coord offset
    lastMoveOffset = indiv.lastMoveDir.asNormalizedCoord()

    # moveX, will be the accumulators that will hold the sum of all the
    # urges to move along each axis. (+- floating values of arbitrary range)
    moveX = isEnabled(Action.MOVE_X) ? actionLevels[Action.MOVE_X] : 0.0
    moveY = isEnabled(Action.MOVE_Y) ? actionLevels[Action.MOVE_Y] : 0.0
    ...
    # Convert the accumulated X, sums to the range -1.0..1.0 and scale by the
    ...
    probX = (int16_t)prob2bool(std.abs(moveX)); # convert abs(level) to 0 or 1
    probY = (int16_t)prob2bool(std.abs(moveY)); # convert abs(level) to 0 or 1

    # The direction of movement (if any) along each axis is the sign
    signumX = moveX < 0.0 ? -1 : 1
    signumY = moveY < 0.0 ? -1 : 1

    # Generate a normalized movement offset, each component is -1, 0, 1
    movementOffset = { (int16_t)(probX * signumX), (int16_t)(probY * signumY)

    # Move there if it's a valid location
    newLoc = indiv.loc + movementOffset
    if grid.isInBounds(newLoc) and grid.isEmptyAt(newLoc):
